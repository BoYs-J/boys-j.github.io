...
function ZeroWeather(data) {
	if ( Number_W <= 18 ) {
		Number_W = Number_W + 1
		let box = data.cloneNode(false)
		if ((!(box instanceof HTMLElement) && !/( )/.test(box.textContent) && !!box.textContent)) {
			divClass = box.textContent
		} else if (/(img)/.test(box.outerHTML)) {
			divClass = box.outerHTML
		} else if (!!divClass) {
			document.querySelector('.nav-plugin-simple').appendChild(box)
			box.innerHTML = divClass
			divClass = ""
		}
	}
}
